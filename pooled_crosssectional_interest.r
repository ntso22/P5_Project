main <- function() {
    ...
    interest <- read.csv('interest.csv', sep=';')
    colnames(interest) <- str_replace_all(colnames(interest), 'X', '')
    interest <- interest %>% as.matrix %>%
        t() %>%
        data.frame()
    ...
    DataSet <- homedata %>%
        ...
        mutate(UdlaansRente = interest[Salgsaar, 1])
    ...
    model <- lm(formula = Pris_Salg ~ Salgsaar +  KommuneNavn + 
        ...
        UdlaansRente + KommuneNavn * UdlaansRente, 
        data = DataSet)
    ...
    write.csv(DataSet, 'clean_data3.csv')
    ...
    write.csv(summary$coefficients, 'linear_model_3.csv')
    return (summary)
}
