BY <- function(data, ftest) {
    ...
    final_model_residuals <- (residuals(final_model))^2

    #Makes Breusch-Pagan formula and executing Breusch-Pagan test
    BP_formula <- paste(
            paste("final_model_residuals", paste(final_variables[! final_variables == "(Intercept)"], collapse=' + '), sep=' ~ ')
    )
    ...
    BP_model <- lm(formula=BP_formula, data=DataSet)
    summary(BP_model)
